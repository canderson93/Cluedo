...
public class UI {
	...
	public void startGame() {
		...
		// main game loop
		gameLoop: while (!game.isFinished()) {
			String in = sc.next();
			...
			case "accuse": // Accusation command
				doAccusation(sc);
				if (!game.isFinished()) { //Continue the game if it isn't over
					nextRound();
				}
				break;
	...
	public void doAccusation(Scanner sc) {
		String character = selectCard(sc, game.getCharacterCards(),
				"Who was the murderer? ");
		String weapon = selectCard(sc, game.getWeaponCards(),
				"What was the weapon?");
		String room = selectCard(sc, game.getRoomCards(),
	...
	public void doSuggestion(Scanner sc) {
		...
		if (game.getCurrent().getLastSuggestion() == room) {
			System.out
					.println("You cannot make two suggestions a row in the same room");
			return;
		}
		...
		if (matched == null) {
			...
			System.out.println("You were shown "
					+ toTitleCase(matched.getValue()));
		}
	...
	private String selectCard(Scanner sc, List<? extends Card> cards, String msg) {
		...
		for (int i = 0; i < cards.size(); i++) {
			System.out.print((i + 1) + ":\t"
					+ toTitleCase(cards.get(i).getValue()));
		...
		while (sc.hasNext()) {
			try {
				...
				System.out.println("You have to select a number between 1 and "
						+ cards.size());
			} catch (NumberFormatException e) {
				System.out
						.println("Input the number of the character to select");
			}
		}
	...
	public void redraw(String msg) {
		...
		System.out.println("Player: " + toTitleCase(p.getName()) + " ("
				+ p.getKey() + ")");
	...
	public static void main(String args[]) {
		...
		while (true) {
			...
			try {
				...
				if (num < 3 || num > 6) {
					System.out
							.println("Please choose a number between 3 and 6");
					continue;
				}
